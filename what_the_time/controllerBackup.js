...
angular.module('timezoneApp.controllers').controller('mainController', function ($rootScope, $scope, $interval, $timeout, $filter, $http) {
	...
	$scope.triggerSearchBtn = function () {
		...
		$timeout(function () {
			...
			$scope.mainTriggerBtnFlag = false;
			if (Number($scope.extractHours) > 06 && Number($scope.extractHours) < 18) {
				$scope.getGrettingImg = "Sun-48.png";
				console.log($scope.getGrettingImg);
			} else {
				$scope.getGrettingImg = "Bright_Moon.png";
				console.log($scope.getGrettingImg);
			}
		}, 2000);
